...
define(function (require, exports) {
  ...
  exports.ready = function (code, data, cite) {
    ...
    let vmData = {
      ...
      // 表单数据
      formdata: {},
      dataUrl: backend.bcmpService + '/bcmpSmDeviceLog/index',
    ...
    const vm = new Vue({
      ...
      // 方法
      methods: {
        downloadFn: function (viewData) {
          var downloadUrl = backend.bcmpService + '/bcmpSmDeviceLog/download?deviceLogId=' +
            viewData.deviceLogId;
        ...
        deleteFn: function (viewData) {
          var _this = this;
          this.$confirm('确定要删除【' + viewData.deviceLogId + '】吗?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(function () {
            // 调用删除提交服务
            var deviceLogId = viewData.deviceLogId;
            yufp.service.request({
              method: 'POST',
              name: backend.adminService + '/bcmpSmDeviceLog/delete/' + deviceLogId,
        ...
        addData: function () {
          ...
          yufp.service.request({
            method: 'POST',
            data: model,
            name: backend.adminService + '/bcmpSmDeviceLog/create',
            callback: function (code, message, response) {
              if (code === 0) {
        ...
        editData: function () {
          ...
          yufp.service.request({
            method: 'POST',
            data: model,
            name: backend.adminService + '/bcmpSmDeviceLog/update',
            callback: function (code, message, response) {
              if (code === 0) {
                vm.$message({
                  message: '数据更新成功！'
