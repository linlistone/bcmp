    ...
    exports.ready = function (code, data, cite) {
        ...
        let vmData = {
            ...
            m_roleGrid: {
                ...
                currentRow: null,
                dataUrl: backend.appOcaService + '/api/adminsmrole/queryByModId',
                dataParams: {
                    condition: JSON.stringify({
                        roleSts: 'A',
                        userId:fox.localsession.getUserInfo().userId
            ...
            //PAD应用资源树
            m_appTree: {
                reourceUrl: backend.appOcaService + '/api/adminsmauthteco/apptreequery?sysId=' + yufp.session.logicSys.id,
                loading: false,
                treeCheckBox: true,
                height: yufp.custom.viewSize().height - 130,
            },
        ...
        const vm = new Vue({
            ...
            methods: {
                //对象类型选择 
                change: function (val) {
                    this.m_select.showObjectFlag = val;
                    this.m_select.sourObjectType = val;
                    this.m_select.builObjectType = val;
                },
                //角色查询
                queryRoleFn: function () {
                    ...
                    var params = {
                        condition: JSON.stringify({
                            orgId: _this.m_roleGrid.query.orgId ? _this.m_roleGrid.query.orgId : null,
                            roleCode: _this.m_roleGrid.query.roleCode ? _this.m_roleGrid.query.roleCode : null,
                            roleName: _this.m_roleGrid.query.roleName ? _this.m_roleGrid.query.roleName : null,
                            roleSts: 'A'
                        })
                    };
                    _this.$refs.roleTable.remoteData(params);
                    _this.$refs.menuTree.setCheckedKeys([]);
                    _this.m_roleGrid.currentRow = null;
                },
                //角色查询条件重置
                resetQueryRoleFn: function () {
                    this.m_roleGrid.query = {
                        orgId: '',
                        roleCode: '',
                        roleName: ''
                    };
                ...
                selectRowRole: function (row) {
                    if (row) {
                        ...
                        this.m_roleGrid.currentRow = row;
                        if (this.activeName == 'padresource') {
                            var param = {
                                objectType: this.m_select.showObjectFlag,
                                objectId: row.roleId,
                                resType: 'A',
                            ...
                            this.m_menuTree.reourceUrl = backend.appOcaService + '/api/adminsmauthteco/menutreerefresh?sysId=' +param.sysId + '&roleId='+param.objectId,
                ...
                orgClickFn: function (nodes) {
                    ...
                    if (nodes != null) {
                        ...
                        this.filterAppText = '';
                        this.ifselectData = true;
                        _this.m_orgGrid.nowNode = nodes;
                        var param = {
                            objectType: _this.m_select.showObjectFlag,
                ...
                queryDptFn: function () {
                    var dptParams = {
                        condition: JSON.stringify({
                            belongOrgId: this.m_dptGrid.query.belongOrgId ? this.m_dptGrid.query.belongOrgId : null,
                            dptCde: this.m_dptGrid.query.dptCde ? this.m_dptGrid.query.dptCde : null,
                            dptName: this.m_dptGrid.query.dptName ? this.m_dptGrid.query.dptName : null,
                            dptSts: 'A'
                        })
                    };
                    var me = this;
                    ...
                    this.m_dptGrid.currentRow = null;
                },
                // 部门查询条件重置
                resetDptRoleFn: function () {
                    this.m_dptGrid.query = {
                        dptCde: '',
                        dptName: '',
                        belongOrgId: ''
                    };
                ...
                //部门表格选择 更新菜单树
                rowClickFnDep: function (row) {
                    if (row != null) {
                        this.filterText = '';
                        this.filterAppText = '';
                        this.ifselectData = true;
                        this.m_dptGrid.currentRow = row;
                        var param = {
                ...
                queryUserFn: function () {
                    this.$refs.userTable.remoteData({
                        condition: JSON.stringify({
                            orgId: this.m_userGrid.query.orgId ? this.m_userGrid.query.orgId : yufp.session.org.id,
                            userInfo: this.m_userGrid.query.userInfo ? this.m_userGrid.query.userInfo : null,
                            userSts: 'A'
                        })
                    });
                    this.$refs.menuTree.setCheckedKeys([]);
                    this.m_userGrid.currentRow = null;
                },
                // 用户查询重置
                resetUserFn: function () {
                    this.m_userGrid.query = {
                        userInfo: '',
                        orgId: ''
                    };
                },
                ...
                selectRowUser: function (row) {
                    if (row) {
                        ...
                        this.filterAppText = '';
                        this.ifselectData = true;
                        this.m_userGrid.currentRow = row;
                        var param = {
                            objectType: this.showObjectFlag,
                            objectId: row.userId,
                            resType: 'M,C',
                ...
                refreshMenuTree: function (param) {
                    ...
                    fox.service.request({
                        ...
                        callback: function (code, message, response) {
                            ...
                            _this.m_menuTree.loading = false;
                            _this.nodeCheckNum = 0;
                        }
                    });
                },
                // 刷新应用树
                refreshAppTree: function (param) {
                    var _this = this;
                    _this.reSetPadFn(); // 重置
                    _this.m_appTree.loading = true;
                    // 发起请求
                    fox.service.request({
                        method: 'GET',
                        url: backend.appOcaService + '/api/adminsmauthteco/queryinfo',
                        ...
                        callback: function (code, message, response) {
                            var infos = [];
                            if (leafAppNodes.length == 0) {
                                _this.getAppNodesWithoutChildren(_this.$refs.appTree.data);
                            }
                            for (var i = 0; i < leafAppNodes.length; i++) {
                                var node = leafAppNodes[i];
                                for (var j = 0; j < response.data.length; j++) {
                                    var d = response.data[j];
                                    if (node['id'] == d['authresId']) {
                                        infos.push(d);
                            ...
                            _this.$refs.appTree.setCheckedKeys(keys);
                            _this.m_appTree.loading = false;
                            _this.nodeCheckNum = 0;
                        }
                    });
                },
                ...
                savePcAllInfoFn() {
                    var _this = this;
                    var objectType = _this.m_select.showObjectFlag;
                    var objectId;
                    var dataInfo = [];
                    var ctrInfo = []; // 控制点数据
                    var dataMap = {};
                    if (objectType === 'R') { // 角色
                        _this.checkObjSelected(_this.m_roleGrid.currentRow);
                        objectId = this.m_roleGrid.currentRow.roleId;
                    } else if (objectType === 'U') { // 用户
                        _this.checkObjSelected(_this.m_userGrid.currentRow);
                        objectId = _this.m_userGrid.currentRow.userId;
                    } else if (objectType === 'D') { // 部门
                        _this.checkObjSelected(_this.m_dptGrid.currentRow);
                        objectId = _this.m_dptGrid.currentRow.dptId;
                    } else if (objectType === 'G') { // 机构
                        _this.checkObjSelected(_this.m_orgTree.nowNode);
                        objectId = _this.m_orgGrid.nowNode.orgId;
                    }
                ...
                // 复制数据
                copyInfoFn: function () {
                    if (this.m_select.sourObjectType === 'R') { // 角色
                        if (this.m_roleGrid.currentRow == null) {
                            this.$message({
                        ...
                        this.m_copyGrid.dialogVisible = true;
                        this.m_copyGrid.sourObjectId = this.m_roleGrid.currentRow.roleId;
                        this.$nextTick(function () {
                            this.$refs.roleTable1.remoteData();
                        });
                    } else if (this.m_select.sourObjectType === 'U') { // 用户
                        if (this.m_userGrid.currentRow == null) {
                            this.$message({
                                message: '请先选择一条待复制对象，然后点击复制',
                                type: 'warning'
                            });
                        ...
                        this.m_copyGrid.dialogVisible = true;
                        this.m_copyGrid.sourObjectId = this.m_userGrid.currentRow.userId;
                        this.$nextTick(function () {
                            this.$refs.userTable1.remoteData();
                        });
                    } else if (this.m_select.sourObjectType === 'D') { // 部门
                        if (this.m_dptGrid.currentRow == null) {
                            this.$message({
                                message: '请先选择一条待复制对象，然后点击复制',
                                type: 'warning'
                            });
                        ...
                        this.m_copyGrid.dialogVisible = true;
                        this.m_copyGrid.sourObjectId = this.m_dptGrid.currentRow.dptId;
                        this.$nextTick(function () {
                            this.$refs.dptTable1.remoteData();
                        });
                    } else if (this.m_select.sourObjectType === 'G') { // 机构
                        if (this.m_orgGrid.nowNode == null || this.m_orgGrid.nowNode == '') {
                            this.$message({
                                message: '请先选择一条待复制对象，然后点击复制',
                                type: 'warning'
                            });
                        ...
                        this.m_copyGrid.dialogVisible = true;
                        this.m_copyGrid.sourObjectId = this.m_orgGrid.nowNode.orgId;
                        this.$nextTick(function () {
                ...
                copyCheck: function () {
                    var _this = this;
                    if (_this.m_select.builObjectType === 'R') { // 角色
                        ...
                        _this.m_copyGrid.builObjectId = _this.$refs.roleTable1.selections[0].roleId;
                    } else if (_this.m_select.builObjectType === 'U') { // 用户
                        ...
                        _this.m_copyGrid.builObjectId = _this.$refs.userTable1.selections[0].userId;
                    } else if (_this.m_select.builObjectType === 'D') { // 部门
                        if (_this.$refs.dptTable1.selections.length == 0) {
                            _this.$message({
                                message: '请先选择粘贴对象',
                                type: 'warning'
                            });
                        ...
                        _this.m_copyGrid.builObjectId = _this.$refs.dptTable1.selections[0].dptId;
                    } else if (_this.m_select.builObjectType === 'G') { // 机构
                        ...
                        _this.m_copyGrid.builObjectId = _this.m_copyGrid.orgTreeNodes.orgId;
                    }
                    var param = {
                        sourObjectType: _this.m_select.sourObjectType,
                        sourObjectId: _this.m_copyGrid.sourObjectId,
                        builObjectType: _this.m_select.builObjectType,
                        builObjectId: _this.m_copyGrid.builObjectId,
                        lastChgUsr: yufp.session.userId,
                        sysId: yufp.session.logicSys.id
                    };
                    // 发起请求
                    fox.service.request({
                        ...
                        callback: function (code, message, response) {
                            ...
                            _this.m_copyGrid.dialogVisible = false;
                        }
                    });
                },
                // 复制时机构树点击事件
                orgClickFn1: function (nodes) {
                    this.m_copyGrid.orgTreeNodes = nodes;
                },
                // 导出数据
                exportInfoFn: function () {
                    ...
                    if (this.ifselectData) {
                        if (this.m_select.showObjectFlag === 'R') { // 角色
                            objectId = this.m_roleGrid.currentRow.roleId;
                        } else if (this.m_select.showObjectFlag === 'U') { // 用户
                            objectId = this.m_userGrid.currentRow.userId;
                        } else if (this.m_select.showObjectFlag === 'D') { // 部门
                            objectId = this.m_dptGrid.currentRow.dptId;
                        } else if (this.m_select.showObjectFlag === 'G') { // 机构
            ...
            mounted: function () {
                var _this = this;
                // _this.m_menuTree.reourceUrl = backend.appOcaService + '/api/adminsmauthteco/menutreequery?sysId=' + yufp.session.logicSys.id,
                fox.lookup.bind('OBJECT_TYPE', function (data) {
                    _this.typeOptions = data;
                });
            },
            watch: {
                filterText: function (val) {
                    val == '' ? this.m_btn.ifTreeButton = false : this.m_btn.ifTreeButton = true;
                    this.$refs.menuTree.filter(val);
                },
                filterAppText: function (val) {
                    val == '' ? this.m_btn.ifPadTreeButton = false : this.m_btn.ifPadTreeButton = true;
