...
define(function (require, exports) {
  ...
  exports.ready = function (hashCode, data, cite) {
    // 创建virtual model
    yufp.custom.vue({
      el: '#el_combo_tree_demo',
      data: {
        height: yufp.custom.viewSize().height - 100,
        singleSelected: '',
        multipSelected: [],
        dataUrl: '/trade/example/tree',
        dataParams: {},
        tableColumns: [
          { label: '编码', prop: 'id' },
        ...
        grid: {
          data1: [
            ...
            { name: 'data-pid', remark: '节点父ID字段', type: 'String', option: '—', default: 'pid' },
            { name: 'data-url', remark: '查询URL', type: 'String', option: '—', default: '—' },
            { name: 'root-visible', remark: '是否显示根节点，当data-root配置值为根节点父ID值时，则需配置为true（懒加载时不用）', type: 'Boolean', option: '—', default: '—' },
            { name: 'data-root', remark: '根节点值或根节点父ID值，当lazy配置为true时，表示根节点父ID值', type: 'string,object', option: '—', default: 'label' }
          ],
          data2: [
            { name: 'getSelectedObjs', remark: '获取选中行的数据对象，多选时返回数据对象数组，单选返回选中的数据对象', param: '-' },
            { name: 'deleteSelected', remark: '清空所有选中节点', param: '-' },
      ...
      methods: {
        ...
        getSelectdText: function () {
          this.$alert(this.$refs.myTreeSelect.selectedLabel);
        },
        getSelectdObjs: function () {
          this.$alert(JSON.stringify(this.$refs.myTreeSelect.getSelectedObjs()));
        },
        resetSelect: function () {
          this.$refs.myTreeSelect.clear();
        },
        ...
        getSelectdText2: function () {
          this.$alert(this.$refs.myTreeSelect2.selectedLabel);
        },
        getSelectdObjs2: function () {
          this.$alert(JSON.stringify(this.$refs.myTreeSelect2.getSelectedObjs()));
        },
        resetSelect2: function () {
          this.$refs.myTreeSelect2.clear();
        }
