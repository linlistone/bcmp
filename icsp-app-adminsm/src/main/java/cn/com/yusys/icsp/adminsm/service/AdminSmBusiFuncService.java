...
import cn.com.yusys.icsp.adminsm.repository.mapper.AdminSmBusiFuncMapper;
...
public class AdminSmBusiFuncService extends BaseService {
	...
	public  AdminSmBusiFunc show(String funcId) throws Exception {
		...
		List<AdminSmBusiFunc> list = index(model);
		if (list == null || list.isEmpty()) {
			throw new ICSPException("数据不存在" + funcId);
		}
		AdminSmBusiFunc smMenu = list.get(0);
		return smMenu;
	}
	 ...
	 * @算法描述: 无
	 */
	public  List<AdminSmBusiFunc> index(QueryModel model)
			throws Exception {
