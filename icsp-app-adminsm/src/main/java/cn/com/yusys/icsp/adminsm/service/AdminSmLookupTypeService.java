...
import cn.com.yusys.icsp.adminsm.repository.mapper.AdminSmLookupTypeMapper;
...
public class AdminSmLookupTypeService extends BaseService {
	...
	public  AdminSmLookupType show(String lookupTypeId) throws Exception {
		...
		List<AdminSmLookupType> list = index(model);
		if (list == null || list.isEmpty()) {
			throw new ICSPException("数据不存在" + lookupTypeId);
		}
		AdminSmLookupType domain = list.get(0);
		return domain;
	}
	 ...
	 * @算法描述: 无
	 */
	public  List<AdminSmLookupType> index(QueryModel model) throws Exception {
