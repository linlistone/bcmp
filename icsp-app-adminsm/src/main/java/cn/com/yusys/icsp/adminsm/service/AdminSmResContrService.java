...
import cn.com.yusys.icsp.adminsm.repository.mapper.AdminSmResContrMapper;
...
public class AdminSmResContrService extends BaseService {
	...
	public  AdminSmResContr show(String contrId) throws Exception {
		...
		List<AdminSmResContr> list = index(model);
		if (list == null || list.isEmpty()) {
			throw new ICSPException("数据不存在" + contrId);
		}
		AdminSmResContr domain = list.get(0);
		return domain;
	}
	 ...
	 * @算法描述: 无
	 */
	public  List<AdminSmResContr> index(QueryModel model) throws Exception {
