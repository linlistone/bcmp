...
import cn.com.yusys.icsp.bcmp.repository.mapper.BcmpCmNodeinfoMapper;
...
public class BcmpCmNodeinfoService extends BaseService {
	...
	public BcmpCmNodeinfo show(String nodeId) throws Exception {
		...
		List<BcmpCmNodeinfo> list = index(model);
		if (list == null || list.isEmpty()) {
			throw new ICSPException("数据不存在" + nodeId);
		}
		BcmpCmNodeinfo bcmpCmNodeinfo = list.get(0);
	 ...
	 * @算法描述: 无
	 */
	public List<BcmpCmNodeinfo> index(QueryModel model)
			throws Exception {
